...
class DocumentProcessingService {
    ...
    async processDocument(request) {
        ...
        try {
            ...
            // Chunk the document
            const chunks = await this.chunkDocument(documentText, request.chunkSize || 1000, request.chunkOverlap || 100);
            ...
            this.insights.trackDocumentProcessing(request.fileName, documentContent.length, duration, true, chunks.length);
            return {
                jobId,
                status: 'completed',
            ...
            this.insights.trackDocumentProcessing(request.fileName, 0, duration, false);
            return {
                jobId,
                status: 'failed',
     ...
     */
    async createProcessingJob(jobId, request) {
        const job = {
            id: jobId,
            type: 'document_job',
            tenantId: request.tenantId,
     ...
     */
    async extractWithDocumentIntelligence(content) {
     ...
     */
    async chunkDocument(text, chunkSize, overlap) {
        const chunks = [];
     ...
     */
    async processChunks(chunks, request, jobId) {
    ...
    async generateEmbedding(text) {
        try {
            const response = await this.openaiClient.getEmbeddings('text-embedding-ada-002', [text]);
     ...
     * Index chunk in Azure AI Search
     */
    async indexChunk(chunk, request, jobId) {
     ...
     */
    async updateProcessingJob(jobId, status, chunks, errorMessage) {
        try {
...
export async function httpTrigger(request, context) {
    ...
    try {
        ...
        // Check permissions
        const hasPermission = await security.checkPermission(authResult.principal, 'document:process', { tenantId: body.tenantId });
        if (!hasPermission) {
            return {
                status: 403,
