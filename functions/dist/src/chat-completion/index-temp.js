...
class ChatCompletionService {
    ...
    async processChatCompletion(request) {
        ...
        try {
            ...
            const conversationHistory = await this.getConversationHistory(request.tenantId, request.userId, sessionId);
            ...
            const messages = this.buildChatMessages(systemPrompt, conversationHistory, request.message);
            // Call Azure OpenAI
            const completion = await this.openaiClient.getChatCompletions(request.model || 'gpt-4', messages, {
            ...
            await this.saveConversation(request.tenantId, request.userId, sessionId, request.message, assistantMessage, contextDocuments, request.model || 'gpt-4');
            // Track metrics
     ...
     */
    async performRAGSearch(request) {
     ...
     */
    async getConversationHistory(tenantId, userId, sessionId) {
        try {
     ...
     */
    buildChatMessages(systemPrompt, history, userMessage) {
        const messages = [
     ...
     * Save conversation to Cosmos DB
     */
    async saveConversation(tenantId, userId, sessionId, userMessage, assistantMessage, sources, model) {
        try {
            const chatDocument = {
                id: uuidv4(),
                type: 'chat',
...
Current date: ${new Date().toISOString().split('T')[0]}`;
     ...
     */
    buildRAGSystemPrompt(documents) {
