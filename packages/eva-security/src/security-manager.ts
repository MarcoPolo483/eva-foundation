...
export class SecurityManager {
    ...
    try {
      ...
      if (this.isIPBlocked(ipAddress)) {
        return this.handleAuthenticationFailure(
          'unknown',
          'unknown',
          'IP address blocked due to multiple failed attempts',
          correlationId,
          ipAddress,
          userAgent
        );
   ...
   */
  private async buildUserContext(userClaims: any): Promise<UserContext> {
  ...
  async checkAuthorization(
    ...
    resource: string,
    action: string,
    resourceContext?: DataAccessContext
    ...
    try {
      ...
      if (resourceContext && !this.canAccessDataClassification(userContext, resourceContext.dataClassification)) {
        ...
        return {
          ...
          error: createApiError(
            'FORBIDDEN',
            `Insufficient clearance for ${resourceContext.dataClassification} data`,
            { required: resourceContext.dataClassification },
            correlationId
          )
        };
      ...
      if (resourceContext?.projectId && !this.hasProjectAccess(userContext, resourceContext.projectId)) {
        return {
          ...
          error: createApiError(
            'FORBIDDEN',
            'No access to specified project',
      ...
      return {
        success: false,
        error: createApiError('INTERNAL_SERVER_ERROR', error.message, { error }, correlationId)
      };
    }
  }
   ...
   */
  private hasPermission(userContext: UserContext, resource: string, action: string): boolean {
   ...
   */
  private canAccessDataClassification(userContext: UserContext, classification: string): boolean {
   ...
   */
  private getUserMaxClearance(roles: UserRole[]): string {
    if (roles.includes(UserRole.GLOBAL_ADMIN)) return 'restricted';
    if (roles.includes(UserRole.TENANT_ADMIN)) return 'confidential';
    if (roles.includes(UserRole.PROJECT_OWNER)) return 'internal';
    return 'public';
  }
   ...
   */
  private hasProjectAccess(userContext: UserContext, projectId: ProjectId): boolean {
    return userContext.projectAccess?.includes(projectId) || 
           userContext.roles.includes(UserRole.GLOBAL_ADMIN) ||
           userContext.roles.includes(UserRole.TENANT_ADMIN);
  }
  ...
  private async handleAuthenticationFailure(
    ...
    return {
      success: false,
      error: createApiError('UNAUTHORIZED', 'Authentication failed', { error }, correlationId)
    };
  }
  ...
  private isIPBlocked(ipAddress?: string): boolean {
    if (!ipAddress || !this.config.maxFailedAttempts) return false;
   ...
   * Map Azure AD roles to EVA roles
   */
  private mapAzureRolesToEVARoles(azureRoles: string[]): UserRole[] {
    const roleMapping: Record<string, UserRole> = {
      'EVA.GlobalAdmin': UserRole.GLOBAL_ADMIN,
      'EVA.TenantAdmin': UserRole.TENANT_ADMIN,
      'EVA.ProjectOwner': UserRole.PROJECT_OWNER,
      'EVA.ProjectMember': UserRole.PROJECT_MEMBER,
      'EVA.EndUser': UserRole.END_USER
    };
   ...
   */
  private calculatePermissions(roles: UserRole[]): PermissionObject[] {
      ...
      switch (role) {
        ...
        case UserRole.TENANT_ADMIN:
          permissions.push(
            ...
            { resource: 'document', actions: ['upload', 'read', 'delete'] },
        ...
        case UserRole.PROJECT_OWNER:
          ...
          break;
        case UserRole.PROJECT_MEMBER:
          permissions.push(
            { resource: 'document', actions: ['upload', 'read'] },
            { resource: 'chat', actions: ['create', 'read'] }
          );
          break;
   ...
   */
  private async getUserProjectAccess(userId: string, tenantId: string): Promise<ProjectId[]> {
