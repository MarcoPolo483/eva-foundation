...
export interface CustomTrace {
  message: string;
  severityLevel?: number; // 0=Verbose, 1=Information, 2=Warning, 3=Error, 4=Critical
  properties?: { [key: string]: string };
}
...
export class ApplicationInsights {
   ...
   */
  private securityTelemetryProcessor(envelope: appInsights.Contracts.Envelope): boolean {
    if (!envelope.data) return true;
   ...
   */
  private mapSeverityLevel(level: number): appInsights.Contracts.SeverityLevel {
    switch (level) {
