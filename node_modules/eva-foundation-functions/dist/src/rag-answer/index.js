...
class RAGAnswerService {
     ...
     */
    async processRAGAnswer(request, tenantId, userId, appId) {
        ...
        try {
            ...
            const relevantDocs = await this.retrieveDocuments(request.projectId, request.message, tenantId, request.template?.source_filter);
            // Step 2: Generate answer using OpenAI
            const answer = await this.generateAnswer(request.message, relevantDocs, request.template);
     ...
     */
    async retrieveDocuments(projectId, query, tenantId, sourceFilter) {
        try {
     ...
     */
    async generateAnswer(question, documents, template) {
...
- If you don't know something, say so clearly`
    ...
    calculateRelevanceScore(query, content) {
        ...
        const matches = queryWords.filter(word => contentWords.some(contentWord => contentWord.includes(word) || word.includes(contentWord)));
...
export async function httpTrigger(request, context) {
    ...
    try {
        ...
        const hasPermission = await security.checkPermission(authResult.principal, 'rag:answer', {
            tenantId,
        ...
        const response = await ragService.processRAGAnswer(body, tenantId, userHeader, appHeader);
