...
class AdminService {
     ...
     */
    async processAdminRequest(request) {
        try {
            // Security check: validate admin permissions
            const hasPermission = await this.validateAdminPermissions(request.tenantId, request.adminUserId);
            if (!hasPermission) {
     ...
     */
    async validateAdminPermissions(tenantId, userId) {
    ...
    async listResources(request) {
        ...
        try {
          ...
          `;
            ...
            const response = await this.cosmosClient.queryDocuments(container, query, parameters, request.tenantId);
            // Track metrics
    ...
    async getSystemStatistics(tenantId) {
        ...
        GROUP BY d.status
      `;
            const jobsResponse = await this.cosmosClient.queryDocuments('processing-jobs', jobsQuery, [{ name: '@tenantId', value: tenantId }], tenantId);
      ...
      `;
            const chatsResponse = await this.cosmosClient.queryDocuments('chats', chatsQuery, [
